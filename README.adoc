...
# build docker image
docker build . -t $DOCKER_PUBLISH_TO/chat-ui-js:latest

# authenticate with your Docker registry
docker login

# push the docker image to your registry
docker push $DOCKER_PUBLISH_TO/chat-ui-js:latest
----
...
* Edit https://github.com/cloudstateio/samples-ui-chat/blob/master/chat-ui/docker-compose.yaml[docker-compose.yaml] to replace the prebuilt image `chat-ui-js` by the docker image you have just published in the previous step.
